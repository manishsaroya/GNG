...
for roadmap in roadmap_types:
	if roadmap == "gng":
		with open(data_save_dic[roadmap] + "{}_gng1208_200.pickle".format(map_type), 'rb') as tf:
			gng_data = pickle.load(tf)
	elif roadmap == "gng_top":
		with open(data_save_dic[roadmap] + "{}_gngtop_1208_200.pickle".format(map_type), 'rb') as tf:
			gngtop_data = pickle.load(tf)
	elif roadmap == "gng_top_feedback":
		with open(data_save_dic[roadmap] + '{}_gngtop_feedback_1208_200.pickle'.format(map_type), 'rb') as tf:
			gngtop_feedback_data = pickle.load(tf)
	elif roadmap == "prm":
		with open(data_save_dic[roadmap] + '{}_prm1208.pickle'.format(map_type), 'rb') as tf:
			prm_data = pickle.load(tf)
	elif roadmap == "prm_dense":
		with open(data_save_dic[roadmap] + '{}_prmdense_2500.pickle'.format(map_type), 'rb') as tf:
			prmdense_data = pickle.load(tf)
	elif roadmap == "prm_dense_hilbert":
		with open(data_save_dic[roadmap] + '{}_prmdense_hilbert4000.pickle'.format(map_type), 'rb') as tf:
...
for i in range(len(gng_data[0])):
	...
	if gng_data[0][i]==False and gngtop_feedback_data[0][i]==True and prmdense_data[0][i]==True:
		index_list.append(i)
	...
	if prmdense_data[0][i]:
		if gngtop_data[0][i]:
			spl_gngtop += (prmdense_data[1][i] / gngtop_data[1][i])
		if gngtop_feedback_data[0][i]:
			spl_gngtop_feedback += (prmdense_data[1][i] / gngtop_feedback_data[1][i])
		if gng_data[0][i]:
			spl_gng += (prmdense_data[1][i] /  gng_data[1][i])
		if prm_data[0][i]:
			spl_prm += (prmdense_data[1][i] /  prm_data[1][i])
		if prmdense_data[0][i]:
			spl_prmdense += (prmdense_data[1][i] /  prmdense_data[1][i])
		if prmdense_hilbert_data[0][i]:
			spl_prmdense_hilbert += (prmdense_data[1][i] / prmdense_hilbert_data[1][i])
		spl_count += 1

print("Path cost")
print("prmdense_cost 4000 mean:", np.mean(prmdense_path_cost), np.std(prmdense_path_cost))
print("prmdense_hilbert_cost 2000 mean:", np.mean(prmdense_hilbert_path_cost), np.std(prmdense_hilbert_path_cost))
...
print("node_explored")
print("prmdense_node_explored 4000mean:", np.mean(prmdense_node_explored), np.std(prmdense_node_explored))
print("prmdense_hilbert_node_explored 2000 mean:", np.mean(prmdense_hilbert_node_explored), np.std(prmdense_hilbert_node_explored))
...
print("spl_prmdense PRM4000", spl_prmdense/spl_count, sum(prmdense_data[0])/sum(prmdense_data[0]))
print("spl_prmdense_hilbert PRM 2000", spl_prmdense_hilbert/spl_count, sum(prmdense_hilbert_data[0])/sum(prmdense_data[0]))
print("spl_gngtop ", spl_gngtop/spl_count, sum(gngtop_data[0])/sum(prmdense_data[0]))
print("spl_gngtop_feedback ", spl_gngtop_feedback/spl_count, sum(gngtop_feedback_data[0])/sum(prmdense_data[0]))
print("spl_gng ", spl_gng/spl_count, sum(gng_data[0])/sum(prmdense_data[0]))
print("spl_prm 1000", spl_prm/spl_count, sum(prm_data[0])/sum(prmdense_data[0]))